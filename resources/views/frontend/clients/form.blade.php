<x-app-layout>
    <x-form.layout :form="$form">
        <div class="row">
            ...
            <div class="col-md-6">
                <x-form.input :label="__('form.website_url')" type="url" name="website_url" value="{{ old('website_url', $form?->website_url) }}" placeholder="https://example.com" />
            </div>
        ...
        <div class="row">
            <div class="col-12">
                <x-form.input :label="__('form.logo')" type="file" name="image" accept="image/*" :initialPreview="$form?->image ? asset($form->image) : null" :initialCaption="$form?->image ? basename($form->image) : null" />
            </div>
        </div>
