<x-app-layout>
    <x-form.layout :form="$form">
        <div class="row">
            <div class="col-md-6">
                <x-form.select :label="__('form.category')" name="category_id" :options="$categories" :selected="old('category_id', $form?->category_id)" required />
            </div>
            <div class="col-md-6">
            ...
            @foreach ($locales as $locale => $language)
                <div @class(['tab-pane fade', 'show active' => $locale === config('app.locale')]) id="locale-{{ $locale }}" role="tabpanel" aria-labelledby="locale-{{ $locale }}-tab">
                    <div class="row">
                        ...
                        <div class="col-md-8 offset-md-2">
                            <x-form.textarea editor :label="__('form.answer')" name="translations[{{ $locale }}][answer]" :value="old('translations.' . $locale . '.answer', $translations[$locale]['answer'] ?? '')" rows="5" required />
                        </div>
                    </div>
                </div>
            @endforeach
