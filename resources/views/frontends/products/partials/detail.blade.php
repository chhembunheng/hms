<div class="accordion-item detail-item" data-detail-index="{{ $detailIndex }}">
    ...
    <div id="collapseDetail{{ $featureIndex }}_{{ $detailIndex }}" class="accordion-collapse collapse">
        <div class="accordion-body">

            <div class="row">
                <div class="col-md-4">
                    <x-form.icon label="{{ __('Icon') }}"
                        name="features[{{ $featureIndex }}][details][{{ $detailIndex }}][icon]" :value="$detail['icon'] ?? ''" />
                </div>
            ...
            <div class="tab-content">
                @foreach ($locales->keys() as $locale)
                    ...
                    @php
                        $translation = collect($detail['translations'] ?? [])->firstWhere('locale', $locale) ?? [];
                    @endphp
                    <div @class([
                        'tab-pane fade',
                        'show active' => $locale === config('app.locale'),
                    ])
                        id="detail-{{ $featureIndex }}-{{ $detailIndex }}-locale-{{ $locale }}"
                        role="tabpanel"
