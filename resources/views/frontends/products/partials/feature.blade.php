    ...
    <div id="collapseFeature{{ $index }}" class="accordion-collapse collapse"
        ...
        <div class="accordion-body">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <x-form.input label="{{ __('Sort') }}"
                        type="number" min="0"
                        name="features[{{ $index }}][sort]"
                        value="{{ $feature['sort'] ?? 0 }}" />
                </div>
            </div>
            ...
            <div class="tab-content">
                @foreach ($locales as $locale => $language)
                    ...
                    ]) id="feature-{{ $index }}-locale-{{ $locale }}" role="tabpanel" aria-labelledby="feature-{{ $index }}-locale-{{ $locale }}-tab">
                        <div class="mb-3 mt-2">
                            <x-form.input 
                                label="Title"
                                name="features[{{ $index }}][translations][{{ $locale }}][title]"
                                value="{{ $feature['translations'][$locale]['title'] ?? '' }}" />

                            <x-form.textarea 
                                label="Description"
                                name="features[{{ $index }}][translations][{{ $locale }}][description]"
                                value="{{ $feature['translations'][$locale]['description'] ?? '' }}" />
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <button type="button"
                class="btn btn-outline-success mt-2 add-detail"
                data-feature="{{ $index }}">
                + Add Detail
            </button>

            <button type="button"
                class="btn btn-outline-danger mt-2 remove-feature float-end">
                Remove Feature
            </button>
