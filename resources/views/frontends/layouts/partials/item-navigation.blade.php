<li class="nav-item @if ($navigation->children->count()) dropdown @endif">
    @php
        $route = Route::has($navigation->route) ? route($navigation->route, ['locale' => app()->getLocale()]) : '#';
        if ($navigation->linked_type && $navigation->linked_id) {
            $route = url('/' . app()->getLocale() . '/' . $navigation->route);
        }
    @endphp
    <a href="{{ $route }}" class="nav-link @if ($navigation->children->count()) dropdown-toggle @endif"
        @if ($navigation->children->count()) id="nav-{{ $navigation->id }}" role="button" data-bs-toggle="dropdown" aria-expanded="false" @endif>
