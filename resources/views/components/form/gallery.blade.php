...
<div class="mb-3">
        ...
        @if ($images && count($images) > 0)
            @foreach ($images as $index => $item)
                <div class="col-md-6 mb-3 gallery-item" data-index="{{ $index }}">
                    <div class="card">
                        <img src="{{ asset($item['url'] ?? '') }}" class="card-img-top"
                            style="max-height: 200px; object-fit: cover;" alt="Gallery image">
                        <div class="card-body">
                            <input type="hidden" name="{{ $name }}[{{ $index }}][url]"
                                value="{{ $item['url'] ?? '' }}">
                            <input type="text" class="form-control form-control-sm mb-2"
                                name="{{ $name }}[{{ $index }}][alt]" placeholder="Alt text"
                                value="{{ $item['alt'] ?? '' }}">
                            <input type="text" class="form-control form-control-sm"
                                name="{{ $name }}[{{ $index }}][label]" placeholder="Label"
                                value="{{ $item['label'] ?? '' }}">
                            <button type="button" class="btn btn-sm btn-danger mt-2 remove-image"
                                onclick="removeGalleryItem(this)"><i class="fa-solid fa-trash"></i></button>
                        </div>
                    </div>
                </div>
            @endforeach
    ...
    <input type="file" id="gallery-file-input" class="d-none" accept="image/*" multiple
        onchange="handleGalleryUpload(event, '{{ $name }}')">
</div>
    ...
    function handleGalleryUpload(event, fieldName) {
        ...
        Array.from(files).forEach((file, i) => {
            ...
            reader.onload = (e) => {
                ...
                div.className = 'col-md-3 mb-3 gallery-item';
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <button type="button" class="btn btn-sm btn-danger mt-2" onclick="removeGalleryItem(this)"><i class="fa-solid fa-trash"></i></button>
                    </div>
                </div>
            `;
