                    ...
                    <div class="card shadow-sm border-0 mb-4">
                        <div class="card-body">
                            <div class="row g-4 align-items-center">
                                ...
                                <div class="col-md-3">
                                    <x-form.input :label="__('form.route')" name="route" value="{{ old('route', $form?->route) }}" placeholder="dashboard" />
                                </div>
                                <div class="col-md-3">
                        ...
                        <div class="card-body">
                            <ul class="nav nav-tabs mb-3" style="border-bottom: 2px solid #e9ecef;">
                                @foreach ($locales as $locale => $language)
                                    <li class="nav-item">
                                        <button type="button" class="nav-link @if ($locale === app()->getLocale()) active @endif" data-bs-toggle="tab" data-bs-target="#locale-{{ $locale }}">
                                            <img src="{{ asset($language['flag']) }}" class="lang-flag me-2" style="height:18px;"> {{ $language['name'] }}
                                        </button>
                                    </li>
                                @endforeach
                            </ul>
                            <div class="tab-content">
                                @foreach ($locales as $locale => $lng)
                                    <div id="locale-{{ $locale }}" class="tab-pane fade @if ($locale === app()->getLocale()) show active @endif">
                                        <div class="row justify-content-center">
                        ...
                        <div class="card-body">
                            ...
                            <div id="menu-permissions-list">
                                ...
                                @foreach ($form->permissions ?? [] as $perm)
                                    ...
                                    @endphp
                                    <div class="permission-row d-flex align-items-start gap-2 my-2 p-2 rounded border bg-light-subtle" draggable="true" style="box-shadow:0 1px 2px #e9ecef;">
                                        <input type="hidden" name="permissions_existing[{{ $perm->id }}][id]" value="{{ $perm->id }}">
                                        <input type="hidden" name="permissions_existing[{{ $perm->id }}][sort]" class="perm-sort-input" value="{{ $perm->sort }}">
                                        <span class="drag-handle btn btn-light btn-sm"><i class="fa-solid fa-grip-lines"></i></span>
                                        <div class="flex-grow-1">
                                            <div class="row g-2">
                                                <div class="col-auto">
                                                    <x-form.icon name="permissions_existing[{{ $perm->id }}][icon]" :text="false" value="{{ $perm->icon }}" />
                                                </div>
                                                <div class="col">
                                                    <div class="input-group input-group-sm">
                                                        <span class="input-group-text perm-action-prefix">{{ $prefix ? $prefix . '.' : '' }}</span>
                                                        <input type="text" class="form-control perm-action-visible" value="{{ $suffix }}" placeholder="edit">
                                                        <input type="hidden" name="permissions_existing[{{ $perm->id }}][action_route]" class="perm-action-hidden" value="{{ $action_route }}">
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <input type="text" name="permissions_existing[{{ $perm->id }}][slug]" class="form-control form-control-sm" value="{{ $perm->slug }}" placeholder="slug-auto" />
                                                </div>
                                            </div>
                                            {{-- Translation Tabs --}}
                                            <ul class="nav nav-tabs my-2">
                                                @foreach ($locales as $lc => $lng)
                                                    <li class="nav-item">
                                                        <button type="button" class="nav-link @if ($loop->first) active @endif" data-bs-toggle="tab" data-bs-target="#p-{{ $perm->id }}-{{ $lc }}">{{ $lng['name'] }}</button>
                                                    </li>
                                                @endforeach
                                            </ul>
                                            <div class="tab-content border rounded p-2 bg-white">
                                                @foreach ($locales as $lc => $lng)
                                                    @php $t = $perm->translations->where('locale', $lc)->first(); @endphp
                                                    <div id="p-{{ $perm->id }}-{{ $lc }}" class="tab-pane fade @if ($loop->first) show active @endif">
                                                        <input type="text" class="form-control form-control-sm" name="permissions_existing[{{ $perm->id }}][translations][{{ $lc }}]" value="{{ $t?->name }}">
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                            ...
                            <button type="button" class="btn btn-outline-primary btn-sm mt-2" id="add-permission-btn">+ {{ __('form.add_permission') }}</button>
                        </div>
                    </div>

                    <div class="text-end">
                        <button class="btn btn-primary px-4 py-2">{{ __('form.save') }}</button>
                    </div>
                </div>
            </div>
        </div>
