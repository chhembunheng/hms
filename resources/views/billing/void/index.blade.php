<x-page-index 
    :title="__('billing.void_invoices')" 
    icon="file-x">

    <x-slot:filters>
        <form method="GET" action="{{ route('billing.void.index') }}" class="row g-3 px-1 py-1">
            <div class="col-md-3">
            ...
            <div class="col-md-2 d-flex align-items-end gap-2">
                ...
                <a href="{{ route('billing.void.index') }}" class="btn btn-sm btn-outline-secondary rounded-2 px-3 d-inline-flex align-items-center gap-1">
                    ...
                    <tr>
                        ...
                        <td class="text-end pe-3">
                            <button type="button" class="btn btn-sm btn-light border p-1 rounded-2 text-primary" title="{{ __('billing.view_details') }}"
                                    onclick="viewVoidDetails({{ $invoice->id }})">
                        ...
                        <td colspan="6" class="text-center py-5 text-muted">
                            <div class="d-flex flex-column align-items-center">
                                <div class="rounded-circle bg-light d-flex align-items-center justify-content-center mb-3" style="width: 56px; height: 56px;">
                                    <i data-lucide="file-x" style="width: 28px; height: 28px; stroke: #94a3b8;"></i>
                                </div>
                                <h6 class="fw-semibold text-secondary mb-1">{{ __('billing.no_void_invoices') }}</h6>
                            </div>
                        </td>
                    </tr>
                @endforelse
...
<!-- Void Details Modal -->
<div class="modal fade" id="voidDetailsModal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header border-bottom py-2">
                <h6 class="modal-title fw-bold">{{ __('billing.void_invoice_details') }}</h6>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body py-4 text-center" id="voidDetailsContent">
                <i data-lucide="file-x" style="width: 48px; height: 48px; stroke: #ef4444;" class="mb-3"></i>
                <h5>{{ __('billing.void_invoice_details') }}</h5>
                <p class="text-muted">{{ __('billing.void_invoice_details_not_available') }}</p>
            </div>
...
@push('scripts')
<script>
function viewVoidDetails(invoiceId) {
    new bootstrap.Modal(document.getElementById('voidDetailsModal')).show();
}
</script>
@endpush
