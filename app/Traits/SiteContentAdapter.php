...
trait SiteContentAdapter
{
    public function getCommonContent(string $locale): array
    {
        ...
        $services = $load('services', fn() => $this->safeQuery(fn() => Service::select('id','slug','icon','image','sort')
            ->with(['translations' => $translationScope])
            ->orderBy('sort')->limit(50)->get()));

        $products = $load('products', fn() => $this->safeQuery(fn() => Product::select('id','slug','image','icon','sort')
            ->with([
                'translations' => $translationScope,
                'features' => function($q){ $q->select('id','product_id'); },
            ])
            ->orderBy('sort')->limit(50)->get()));
        ...
        $pricing = $load('pricing', fn() => $this->safeQuery(fn() => Plan::select('id','sort')
            ->with([
                'translations' => $translationScope,
                'features' => function($q){ $q->select('id','plan_id'); },
            ])
            ->orderBy('sort')->limit(50)->get()));
