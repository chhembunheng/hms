...
class FloorDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('floor_number', fn($row) => $row->floor_number)
            ...
            ->editColumn('created_at', function (Floor $model) {
                return $model->created_at?->format(config('init.datetime.display_format'));
            })
            ->addColumn('action', fn($row) => view('rooms.floors.action', compact('row')))
            ...
            if (is_array($filters)) {
                if (!empty($filters['search'])) {
                    $query->where(function ($q) use ($filters) {
                        $q->where('floor_number', 'like', '%' . $filters['search'] . '%')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('floor_number')->title(__('rooms.floor_number')),
    ...
    {
        return __('rooms.floors') . '_' . date('YmdHis');
    }
}
